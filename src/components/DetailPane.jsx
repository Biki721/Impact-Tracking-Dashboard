...
function DetailPane({ record, onEdit, onCopyBullet, onExportPDF }) {
  if (!record) {
    return (
      <section className="hidden md:flex md:flex-1 flex-col bg-slate-950/40">
  ...
  return (
    <section className="hidden md:flex md:flex-1 flex-col bg-slate-950/40">
      <header className="flex items-center justify-between px-4 py-2 border-b border-slate-800 text-[11px]">
        <div>
          <h2 className="text-sm font-semibold text-slate-50">{record.projectName}</h2>
          <p className="text-[10px] text-slate-400">
            {record.category} • {record.status} • {record.startDate} → {record.endDate || '—'}
          </p>
          {techSummary && <p className="text-[10px] text-slate-500 mt-0.5">Tech: {techSummary}</p>}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 text-[11px] space-y-4">
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Problem</h3>
          <p className="text-slate-200 mb-1">{record.problem?.what}</p>
          {record.problem?.why && <p className="text-slate-400">Why: {record.problem.why}</p>}
        </section>

        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Contribution</h3>
          {record.contribution?.summary && (
            <p className="text-slate-200 mb-1">{record.contribution.summary}</p>
          )}
          {record.contribution?.responsibilities?.length ? (
            <div className="flex flex-wrap gap-1.5 mt-1">
              {record.contribution.responsibilities.map((resp) => (
                <span
                  key={resp}
                  className="rounded-full bg-slate-900 border border-slate-700 px-2 py-0.5 text-[10px] text-slate-200"
                >
                  {resp}
                </span>
              ))}
        ...
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Quantifiable impact</h3>
                ...
                <tbody className="divide-y divide-slate-800 text-slate-200">
                  {record.impact.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-2 py-1.5 align-top text-slate-300">{row.before}</td>
                      <td className="px-2 py-1.5 align-top text-slate-300">{row.after}</td>
                      <td className="px-2 py-1.5 align-top text-emerald-300">{row.improvement}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Before vs After</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded-md border border-slate-800 bg-slate-950/60 p-2">
              <div className="text-[10px] font-semibold text-slate-400 mb-1">Before</div>
              <p className="text-slate-200 whitespace-pre-wrap min-h-[60px]">
                {record.beforeText || '—'}
              </p>
            </div>
            <div className="rounded-md border border-slate-800 bg-slate-950/60 p-2">
              <div className="text-[10px] font-semibold text-slate-400 mb-1">After</div>
              <p className="text-slate-200 whitespace-pre-wrap min-h-[60px]">
                {record.afterText || '—'}
              </p>
            </div>
        ...
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">AI / automation value add</h3>
          {record.aiValueAdd?.features?.length || record.aiValueAdd?.outcome ? (
            <div>
              ...
              {record.aiValueAdd.outcome && (
                <p className="text-slate-300">{record.aiValueAdd.outcome}</p>
              )}
            </div>
          ) : (
        ...
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Evidence / proof</h3>
              ...
              {record.evidence.map((item, idx) => {
                if (item.type === 'upload' && item.mimeType?.startsWith('image/')) {
                  return (
                    <div
                      key={idx}
                      className="w-20 h-16 rounded-md border border-slate-700 bg-slate-900 overflow-hidden flex items-center justify-center"
                    >
                      <img
                        src={item.url}
        ...
        <section>
          <h3 className="text-xs font-semibold text-slate-300 mb-1.5">Feedback</h3>
          {record.feedback?.length ? (
            <div className="space-y-1.5">
              {record.feedback.map((fb, idx) => (
                ...
                >
                  <MessageCircle className="h-3.5 w-3.5 text-slate-500 mt-0.5" />
                  <div>
